...
`Laminas\ConfigAggregator\ConfigAggregator` (and therefore by Mezzio). No manual config file is
needed.
...
- `StrategyInterface` -> `Strategy\HandleStrategy` (the default; alias it to
  `Strategy\ClassnameStrategy` to opt into named handler methods)
...
2. Define its handler (implementing `CommandHandlerInterface` or `QueryHandlerInterface`). These are
   marker interfaces with no required method; declare the method your wired strategy expects (`handle()`
